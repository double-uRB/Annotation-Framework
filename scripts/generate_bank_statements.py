...
def draw_statement(doc_index: int, out_dir: Path, rng: random.Random) -> dict[str, Any]:
    ...
    return {
        ...
        "quality_metadata": {
            "ocr_confidence": 0.78 if "OCR_NOISY" in quality_tags else 0.97,
            "difficulty_rating": 4 if any(tag in quality_tags for tag in ["OCR_NOISY", "MULTI_CURRENCY", "INCOMPLETE"]) else 3,
            "time_to_annotate": rng.randint(420, 900),
